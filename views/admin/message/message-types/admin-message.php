...
<div class="order-message order-message--<?php echo esc_attr( $message->getMessageType()->getName() ); ?>"
	...
	<div class="order-message__content">
		...
		<?php
			...
			ServiceContainer::getInstance()->getFileManager()->includeTemplate( 'admin/message/message-attachment.php',
	...
	<p class="order-message-meta">
		<?php if ( $message->getDateRead() ) : ?>
			...
			<?php
				...
				printf( esc_attr__( 'The message was read at %s' ),
					esc_html( $message->getDateRead()->format( 'Y-m-d H:i:s' ) ) );
			?>
			">✓</span>
		<?php endif; ?>
		<abbr class="exact-date"
			...
			<?php
				...
				printf( /* translators: $1: Date created, $2 Time created */ esc_html__( 'Sent on %1$s at %2$s ',
						'order-messenger-for-woocommerce' ), esc_html( date_i18n( wc_date_format(),
						strtotime( $message->getDateSent()->format( 'y-m-d H:i:s' ) ) ) ),
					esc_html( date_i18n( wc_time_format(),
						strtotime( $message->getDateSent()->format( 'y-m-d H:i:s' ) ) ) ) );
			?>
		</abbr>
